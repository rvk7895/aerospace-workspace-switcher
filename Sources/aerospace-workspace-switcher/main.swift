...
class RowView: NSView {
    ...
    func configure(_ ws: WorkspaceInfo, selected: Bool) {
        ...
        if ws.hasWindows {
            appLabel.stringValue = ws.appSummary
            appLabel.textColor = TEXT_PRIMARY

            let joined = ws.titleSummary
...
class Controller: NSObject, NSTextFieldDelegate {
    ...
    var defaultFiltered: [WorkspaceInfo] {
        WorkspaceFilter.defaultFilter(all)
    }

    func show() {
        all = fetchWorkspaces(cli: cli)
    ...
    func selectCurrent() {
        ...
        switchToWorkspace(name, cli: cli)
        exit(0)
    }
    ...
    func filter(_ q: String) {
        if q.isEmpty {
            ...
            filtered = WorkspaceFilter.search(all, query: q)
        }
        selectedIndex = 0
        rebuildRows()
    }
